...
| Per-turn request assembly  | Selected agent, agent prompt, and effective permissions                  | partial  | The runner includes the selected agent's static system text and materializes tools with its permissions. Complete remaining effective request-policy/options parity. |
...
Provider first-event and inactivity watchdogs are implemented in `packages/core/src/session/runner/llm.ts`. Configuration keys `session.first_event_timeout_ms` and `session.inactivity_timeout_ms` each default to 300,000 ms. These bound waiting for the first stream event and the gap between subsequent events; they are not an absolute task deadline or a currency budget. See `packages/core/src/config/session.ts` and `packages/core/test/session-provider-timeout.test.ts`. Broader provider retry policy remains separate from these watchdogs and must preserve the no-silent-replay rule for interrupted side effects.
...
A process-global `SessionRunCoordinator` serializes execution for each local Session while allowing different Sessions to run concurrently. Resumes join active execution, overlapping wakes coalesce into one follow-up, and interruption stops current process-local execution without deleting durable inbox work. The runner enters the Session's current Location when execution starts and fences each new provider turn against that Location. V2 execution is single-process only: do not expose it from a topology where more than one process can serve the same workspace until durable ownership, fencing, and distributed interruption exist.
...
Each V2 provider turn admits at most `session.tool_call_limit` local tool calls (default 16) and executes at most `session.tool_concurrency` at once (default 4). Calls over the admission limit receive a durable model-visible tool failure; admitted work remains cancellable and all settled work completes before continuation. `ToolOutputStore` separately limits contextual output to 2,000 lines and 50 KiB while retaining oversized full output. The `session.next.*` event schemas remain experimental and unshipped; databases created by earlier experimental builds are disposable rather than compatibility targets.

## V2 Release Gate

Do not make V2 the default product path until the V1 runtime-context parity table is complete for the exposed feature set and paired V1/V2 request fixtures cover the selected agent, provider baseline, effective policy, tool definitions, prompt references, and plugin transforms. The current compatibility adapter must continue to use V1 while any required entry remains `partial` or `missing`.
