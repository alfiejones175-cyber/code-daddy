...
import type { ToolDefinition } from "@opencode-ai/plugin/v2/effect/tool"
        ...
        const context2: PluginContext = {
          ...
          tool: {
            register: <Input, Output>(
              definitions: Record<string, import("@opencode-ai/plugin/v2/promise").ToolDefinition<Input, Output>>,
            ) =>
              register(
                host.tool.register(
                  Object.fromEntries(
                    Object.entries(definitions).map(([name, definition]) => [
                      ...
                      {
                        ...definition,
                        execute: (input: Input, toolContext) =>
                          Effect.tryPromise({
                            try: () => Promise.resolve(definition.execute(input, toolContext)),
                            catch: (error) => error,
                          }),
                      },
                    ]),
                  ) as Record<string, ToolDefinition<Input, Output>>,
                ),
              ),
          },
        }
