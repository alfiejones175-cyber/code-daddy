...
export const make = Effect.fn("PluginHost.make")(function* (plugin: PluginV2.Interface) {
  ...
  const permissions = yield* Effect.serviceOption(PermissionV2.Service)
    ...
    tool: {
      register: (definitions) => {
        if (Option.isNone(tools))
          return Effect.die("Plugin tools are unavailable before the tool registry is initialized")
        if (Option.isNone(permissions))
          return Effect.die("Plugin tools are unavailable before permissions are initialized")
        return Effect.gen(function* () {
                    ...
                    Tool.make({
                      ...
                      execute: (input, context) =>
                        permissions.value
                          ...
                          .pipe(
                            Effect.mapError(
                              () => new Tool.Failure({ message: `Permission denied for plugin tool ${name}` }),
                            ),
                            Effect.andThen(
                              Effect.try({
                                try: () => definition.execute(input, context),
                                catch: (error) =>
          ...
          return { dispose: Scope.close(child, Exit.void).pipe(Effect.ignore) }
        })
      },
    },
  } satisfies Interface
