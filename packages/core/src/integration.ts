...
export class AttemptNotFoundError extends Schema.TaggedErrorClass<AttemptNotFoundError>()(
  "Integration.AttemptNotFound",
  { attemptID: AttemptID },
) {}

export class AttemptExpiredError extends Schema.TaggedErrorClass<AttemptExpiredError>()(
  "Integration.AttemptExpired",
  { attemptID: AttemptID },
) {}

export class AttemptCancelledError extends Schema.TaggedErrorClass<AttemptCancelledError>()(
  "Integration.AttemptCancelled",
  { attemptID: AttemptID },
) {}

export type Error = CodeRequiredError | AuthorizationError | AttemptNotFoundError | AttemptExpiredError | AttemptCancelledError
...
type TerminalAttempt = {
  status: "complete" | "failed" | "expired" | "cancelled"
  message?: string
  removeAt: number
  time: AttemptTime
}
    ...
    const resolveConnections = (entry: Entry | undefined, saved: readonly Credential.Info[]) => {
      const credentials = saved
        .map((credential) => ({
          ...
          authType: credential.value.type,
    ...
    return Service.of({
      ...
      attempt: {
        status: Effect.fn("Integration.attempt.status")(function* (attemptID) {
          ...
          if (!attempt) return { status: "not_found" as const, attemptID }
        ...
        complete: Effect.fn("Integration.attempt.complete")(function* (input) {
          const current = (yield* SynchronizedRef.get(attempts)).get(input.attemptID)
          if (!current) return yield* new AttemptNotFoundError({ attemptID: input.attemptID })
          if (current.status === "expired") return yield* new AttemptExpiredError({ attemptID: input.attemptID })
          if (current.status === "cancelled") return yield* new AttemptCancelledError({ attemptID: input.attemptID })
          if (current.status !== "pending") return
          const attempt = yield* SynchronizedRef.modify(attempts, (current) => {
            const match = current.get(input.attemptID)
            if (!match || match.status !== "pending" || match.completing) return [undefined, current]
        ...
        cancel: Effect.fn("Integration.attempt.cancel")(function* (attemptID) {
          ...
          const attempt = yield* SynchronizedRef.modify(attempts, (current) => {
            ...
            const next = new Map(current)
            next.set(attemptID, { status: "cancelled", time: match.time, removeAt: now + terminalRetention })
            return [match, next]
          })
          if (attempt?.status === "pending") yield* Scope.close(attempt.scope, Exit.void)
        }),
      },
    })
  }),
