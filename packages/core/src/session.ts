...
export class ChildConflictError extends Schema.TaggedErrorClass<ChildConflictError>()("Session.ChildConflictError", {
  sessionID: SessionSchema.ID,
...
export interface Interface {
  ...
  readonly create: (input: CreateInput) => Effect.Effect<SessionSchema.Info, NotFoundError | ChildConflictError>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const result = Service.of({
        ...
        if (recorded) {
          if (
            ...
            (recorded.parentID !== input.parentID ||
              recorded.location.directory !== input.location.directory ||
              recorded.location.workspaceID !== input.location.workspaceID)
          )
            return yield* new ChildConflictError({
              sessionID,
              message: "Child identity is already linked to another parent or Location",
            })
          return recorded
        }
        if (input.parentID) {
          ...
          if (
            input.parentID === sessionID ||
            parent.location.directory !== input.location.directory ||
            parent.location.workspaceID !== input.location.workspaceID
          )
            return yield* new ChildConflictError({
              sessionID,
              message: "Children must inherit their parent's Location",
            })
        }
      ...
      recovery: Effect.fn("V2Session.recovery")(function* (sessionID) {
        yield* result.get(sessionID)
        if ((yield* execution.active).has(sessionID)) return SessionRecovery.Status.make({ type: "running" })
        return yield* SessionRecovery.inspect(db, sessionID)
      }),
