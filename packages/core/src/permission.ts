...
const layer = Layer.effect(
  ...
  EffectRuntime.gen(function* () {
    ...
    const evaluateInput = EffectRuntime.fnUntraced(function* (input: AssertInput) {
      ...
      const inherited: Permission.Ruleset[] = [rules]
      const seen = new Set<SessionV2.ID>([input.sessionID])
      let session = yield* sessions.get(input.sessionID)
      // Delegation cannot grant an action that an ancestor's agent could not perform.
      // Evaluate each policy separately: concatenating them would let a later allow override a deny.
      while (session?.parentID) {
        if (seen.has(session.parentID)) return { effect: "deny" as const, rules: missingAgentPermissions }
        seen.add(session.parentID)
        session = yield* sessions.get(session.parentID)
        if (
          !session ||
        ...
        inherited.push(yield* configured(session.id))
      }
      const blocked = inherited.find((policy) => denied(input, policy))
      if (blocked) return { effect: "deny" as const, rules: blocked }
      const saved = yield* savedRules()
      const effects = inherited.flatMap((policy) =>
        input.resources.map((resource) => evaluate(input.action, resource, policy, saved).effect),
      )
      const effect: Permission.Effect = effects.includes("deny") ? "deny" : effects.includes("ask") ? "ask" : "allow"
      return { effect, rules: inherited.flat().concat(saved) }
    })
