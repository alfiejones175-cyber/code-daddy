...
import { AnthropicMessages } from "@opencode-ai/llm/protocols/anthropic-messages"
...
export const fromCatalogModel = (
  ...
  if (credential?.type === "oauth" && ChatGPT.credential(credential)) {
    if (
      resolved.providerID !== ProviderV2.ID.openai ||
      resolved.api.type !== "aisdk" ||
      resolved.api.package !== "@ai-sdk/openai" ||
      !ChatGPT.supported(resolved)
    )
      return Effect.fail(
        new UnsupportedApiError({ providerID: resolved.providerID, modelID: resolved.id, api: "chatgpt-codex" }),
      )
    // Subscription credentials must never fall through to an API-key endpoint.
    return Effect.succeed(
      withDefaults(resolved, ChatGPT.route(credential))
        .with({ endpoint: { baseURL: "https://chatgpt.com/backend-api/codex" } })
        .model({ id: resolved.api.id }),
    )
  }
  if (resolved.api.type === "aisdk" && resolved.api.package === "@openrouter/ai-sdk-provider") {
    return Effect.gen(function* () {
      const { route } = yield* Effect.promise(() => import("@opencode-ai/llm/providers/openrouter"))
      return withDefaults(resolved, route)
