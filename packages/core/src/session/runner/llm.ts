 ...
 * Recovery is derived from durable prompt and message projections and only starts through explicit resume.
 *
  ...
  Effect.gen(function* () {
    ...
    const runTurnAttempt = Effect.fn("SessionRunner.runTurn")(function* (
      ...
      const metadata = yield* db
        .select({ value: SessionTable.metadata })
        .from(SessionTable)
        .where(eq(SessionTable.id, session.id))
        .get()
        .pipe(Effect.orDie)
      const configuredMaxTurns =
        typeof metadata?.value?.maxTurns === "number"
          ? metadata.value.maxTurns
          : typeof metadata?.value?.maxTurns === "string" && /^\d+$/.test(metadata.value.maxTurns)
            ? Number(metadata.value.maxTurns)
            : undefined
      const taskMaxTurns =
        configuredMaxTurns !== undefined &&
        Number.isInteger(configuredMaxTurns) &&
        configuredMaxTurns >= 1 &&
        configuredMaxTurns <= 100
          ? configuredMaxTurns
          : undefined
      const maxSteps =
        agent.info?.steps === undefined
          ? taskMaxTurns
          : taskMaxTurns === undefined
            ? agent.info.steps
            : Math.min(agent.info.steps, taskMaxTurns)
      ...
      const providerStream = withProviderTimeouts(llm.stream(request), providerTimeouts).pipe(
      ...
      return yield* Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
          ...
          const providerFailure =
            failure instanceof LLMError || failure instanceof ProviderTimeoutError ? failure : undefined
          if (providerFailure && !publisher.hasProviderError()) {
            yield* withPublication(publisher.failUnsettledTools("Provider did not return a tool result", true))
            yield* withPublication(
              publisher.failAssistant(
                providerFailure instanceof LLMError ? providerFailure.reason.message : providerFailure.message,
              ),
            )
          }
...
export const withProviderTimeouts = <A, E, R>(
  stream: Stream.Stream<A, E, R>,
  timeouts: { readonly firstEventMs: number; readonly inactivityMs: number },
) =>
  Stream.transformPull(stream, (pull) =>
    Effect.sync(() => {
      let first = true
      return Effect.suspend(() => {
        const phase = first ? "first_event" : "inactivity"
        const timeoutMs = first ? timeouts.firstEventMs : timeouts.inactivityMs
        return pull.pipe(
          Effect.timeoutOrElse({
            duration: Duration.millis(timeoutMs),
            orElse: () => Effect.fail(new ProviderTimeoutError({ phase, timeoutMs })),
          }),
          Effect.tap(() =>
            Effect.sync(() => {
              first = false
