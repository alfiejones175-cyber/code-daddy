...
import { ConfigSession } from "@opencode-ai/core/config/session"
...
describe("SessionRunnerLLM", () => {
  it.effect("derives explicit recovery state from durable prompt admission and promotion", () =>
    Effect.gen(function* () {
      yield* setup
      const session = yield* SessionV2.Service
      const events = yield* EventV2.Service
      ...
      yield* session.prompt({
        sessionID,
        prompt: Prompt.make({ text: "Recover after restart" }),
        delivery: "queue",
        resume: false,
      })

      expect(yield* session.recovery(sessionID)).toEqual({ type: "pending", delivery: "queue" })

      yield* SessionInput.promoteNextQueued(db, events, sessionID)
      expect(yield* session.recovery(sessionID)).toEqual({ type: "needs_recovery", reason: "promoted_input" })
    }),
  )
  ...
  it.effect("durably fails local tools left running by a prior process before continuing", () =>
    Effect.gen(function* () {
      ...
      expect(yield* session.recovery(sessionID)).toEqual({
        type: "needs_recovery",
  ...
  it.effect("interrupts a blocked provider turn without local tool execution", () =>
    Effect.gen(function* () {
      ...
      expect(yield* session.recovery(sessionID)).toEqual({ type: "idle" })
      yield* session.interrupt(sessionID)
    }),
  )
  ...
  it.effect("enforces a persisted child maxTurns string on the first provider turn", () =>
    Effect.gen(function* () {
      yield* setup
      const agents = yield* AgentV2.Service
      yield* agents.transform((editor) =>
      ...
      yield* db
        .update(SessionTable)
        .set({ metadata: { task: "bounded child", maxTurns: "1" } })
        .where(eq(SessionTable.id, sessionID))
        .run()
      const session = yield* SessionV2.Service
      yield* session.prompt({
        sessionID,
        prompt: Prompt.make({ text: "Finish within the child budget" }),
        resume: false,
      })
      requests.length = 0
      response = [
  ...
  it.effect("times out a silent provider and durably settles the assistant step", () =>
    Effect.gen(function* () {
      yield* setup
      const session = yield* SessionV2.Service
      yield* session.prompt({ sessionID, prompt: Prompt.make({ text: "Do not hang forever" }), resume: false })
      ...
      expect(yield* Fiber.join(run).pipe(Effect.flip)).toMatchObject({
        _tag: "SessionRunner.ProviderTimeoutError",
        phase: "first_event",
        timeoutMs: 100,
      })
      expect(yield* session.context(sessionID)).toMatchObject([
        { type: "user", text: "Do not hang forever" },
        {
          type: "assistant",
          finish: "error",
          error: { type: "unknown", message: "Provider did not send its first event within 100ms" },
        },
      ])
      expect(yield* session.recovery(sessionID)).toEqual({ type: "idle" })
    }),
