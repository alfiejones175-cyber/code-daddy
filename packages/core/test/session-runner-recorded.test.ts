...
for (const catalogRoute of process.env.RECORD === "true" ? [false] : [false, true]) {
  const models = SessionRunnerModel.layerWith(() => {
    if (!catalogRoute) return Effect.succeed(model)
    // Reuse the Chat Completions cassette to exercise the actual V2 runner with
    // OpenRouter's native catalog mapping. No request is sent to this fixture URL.
    return SessionRunnerModel.fromCatalogModel(
      ModelV2.Info.make({
        ...ModelV2.Info.empty(ProviderV2.ID.openrouter, ModelV2.ID.make("gpt-4o-mini")),
        api: {
          type: "aisdk",
      ...
      Effect.map((resolved) =>
        resolved.route.with({ generation: { maxTokens: 20, temperature: 0 } }).model({ id: resolved.id }),
      ),
    )
  })
  ...
  describe("SessionRunnerLLM recorded", () => {
    it.effect(
      catalogRoute
        ? "executes a V2 prompt using the OpenRouter native catalog route"
