...
describe("SessionV2.create", () => {
  ...
  it.effect("records direct children and lists only the requested parent across pages", () =>
    Effect.gen(function* () {
      ...
      const first = yield* sessions.create({
        id: SessionV2.ID.make("ses_child_a"),
        location,
        parentID: parent.id,
        title: "First child",
      })
      const second = yield* sessions.create({
        id: SessionV2.ID.make("ses_child_b"),
        location,
        parentID: parent.id,
        title: "Second child",
      })
      yield* sessions.create({ location, parentID: other.id })
      expect((yield* sessions.list({ parentID: null })).map((session) => session.id).sort()).toEqual(
        [parent.id, other.id].sort(),
      )
      const page = yield* sessions.list({ parentID: parent.id, order: "asc", limit: 1 })
      ...
      expect(
        yield* sessions.list({
          ...
          order: "asc",
          limit: 1,
          anchor: { id: first.id, time: DateTime.toEpochMillis(first.time.created), direction: "next" },
        }),
      ).toEqual([second])
    }),
  )

  it.effect("rejects missing parents, cross-Location children and conflicting child identity reuse", () =>
    Effect.gen(function* () {
      const sessions = yield* SessionV2.Service
      const parent = yield* sessions.create({ location })
      const other = yield* sessions.create({ location })
      ...
      expect(
        (yield* sessions.create({ location, parentID: SessionV2.ID.make("ses_missing") }).pipe(Effect.flip))._tag,
      ).toBe("Session.NotFoundError")
      expect(
        (yield* sessions
          .create({ location: Location.Ref.make({ directory: AbsolutePath.make("/other") }), parentID: parent.id })
          .pipe(Effect.flip))._tag,
      ).toBe("Session.ChildConflictError")
      expect((yield* sessions.create({ id: child.id, location, parentID: other.id }).pipe(Effect.flip))._tag).toBe(
        "Session.ChildConflictError",
      )
      expect((yield* sessions.create({ id: parent.id, location, parentID: child.id }).pipe(Effect.flip))._tag).toBe(
        "Session.ChildConflictError",
      )
    }),
  )
