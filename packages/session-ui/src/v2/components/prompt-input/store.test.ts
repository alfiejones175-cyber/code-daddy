...
describe("prompt input v2 store", () => {
  ...
  test("appends dictation without flattening mentions or attachments", () => {
    const [state, setState] = createStore<PromptInputV2PersistedState>({
      prompt: [
        { type: "text", content: "Review ", start: 0, end: 7 },
        { type: "file", path: "one", content: "@one", start: 7, end: 11 },
        { type: "text", content: " please", start: 11, end: 18 },
        {
          type: "image",
          id: "attachment-1",
          filename: "notes.txt",
    ...
    prompt.appendText("tomorrow")

    expect(prompt.state.prompt).toEqual([
      { type: "text", content: "Review ", start: 0, end: 7 },
      { type: "file", path: "one", content: "@one", start: 7, end: 11 },
      { type: "text", content: " please tomorrow", start: 11, end: 27 },
      {
        type: "image",
        id: "attachment-1",
        filename: "notes.txt",
