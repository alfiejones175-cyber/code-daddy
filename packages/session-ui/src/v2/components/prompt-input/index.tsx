        ...
        <div data-slot="prompt-input-v2-toolbar" class="flex h-11 items-center px-2">
          ...
          >
            <PromptInputV2AddMenu
              ...
              closeLabel={i18n.t("ui.common.close")}
...
export function PromptInputV2AddMenu(props: {
  ...
  const items = [
    {
      label: props.attachLabel,
      shortcut: props.attachShortcut,
      icon: <Icon name="photo" size="small" />,
      onSelect: props.onAttach,
      x: "2px",
      y: "14px",
    },
    {
      label: props.commandsLabel,
      shortcut: "/",
      icon: <span aria-hidden="true">/</span>,
      onSelect: props.onCommands,
      x: "47px",
      y: "25px",
    },
    {
      ...
      shortcut: "@",
      icon: <span aria-hidden="true">@</span>,
      onSelect: props.onContext,
      x: "82px",
      y: "55px",
    },
    {
      label: props.shellLabel,
      shortcut: "!",
      icon: <Icon name="terminal" size="small" />,
      onSelect: props.onShell,
      x: "100px",
      y: "94px",
    },
  ]
    ...
    >
      ...
      >
        <MenuV2.Trigger
          ...
          aria-label={state.open ? props.closeLabel : props.title}
        />
        <MenuV2.Portal>
          <MenuV2.Content class="prompt-input-v2-circle-menu" aria-label={props.title}>
            <For each={items}>
              {(item, index) => (
                <MenuV2.Item
                  class="prompt-input-v2-circle-menu-item"
                  style={{
                    "--circle-menu-index": String(index()),
                    "--circle-menu-x": item.x,
                    "--circle-menu-y": item.y,
                  }}
                  aria-label={item.label}
                  onSelect={item.onSelect}
                >
                  <span data-slot="circle-menu-icon">{item.icon}</span>
                  <span data-slot="circle-menu-label">
                    <span>{item.label}</span>
                    <span data-slot="circle-menu-shortcut">{item.shortcut}</span>
                  </span>
                </MenuV2.Item>
              )}
            </For>
