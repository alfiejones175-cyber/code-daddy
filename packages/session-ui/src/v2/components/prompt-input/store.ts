  ...
  return {
    ...
    appendText(content: string) {
      ...
      const current = prompt
        ...
        .join("")
      const suffix = current && !/\s$/.test(current) ? ` ${content}` : content
      const cursor = promptLength(prompt)
      batch(() => {
        setStore()("prompt", (prompt) => insertText(prompt, cursor, suffix))
        setStore()("cursor", cursor + suffix.length)
      })
    },
    reset() {
      batch(() => {
