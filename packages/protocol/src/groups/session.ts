  ...
  HttpApiGroup.make("server.session")
    ...
    .add(
      HttpApiEndpoint.get("session.recovery", "/api/session/:sessionID/recovery", {
        params: { sessionID: Session.ID },
        success: Schema.Struct({ data: SessionRecovery.Status }),
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            summary: "Inspect session recovery status",
            description:
              "Combine process-local execution ownership with durable Session state to identify work that can be resumed explicitly.",
          }),
        ),
    )
    .add(
      HttpApiEndpoint.post("session.resume", "/api/session/:sessionID/resume", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description:
              "Explicitly start or join one process-local Session drain. Promoted provider work is never replayed automatically.",
          }),
        ),
    )
    .add(
