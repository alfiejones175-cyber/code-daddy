                  ...
                  message:
                    error._tag === "Integration.CodeRequired"
                      ...
                      : error._tag === "Integration.AttemptNotFound"
                        ? "OAuth attempt was not found; start a new attempt"
                        : error._tag === "Integration.AttemptExpired"
                          ? "OAuth attempt expired; start a new attempt"
                          : error._tag === "Integration.AttemptCancelled"
                            ? "OAuth attempt was cancelled; start a new attempt"
                            : "Authentication failed",
                  kind:
                    error._tag === "Integration.CodeRequired"
                      ? "integration_code_required"
                      : error._tag === "Integration.AttemptNotFound"
                        ? "integration_attempt_not_found"
                        : error._tag === "Integration.AttemptExpired"
                          ? "integration_attempt_expired"
                          : error._tag === "Integration.AttemptCancelled"
                            ? "integration_attempt_cancelled"
