  ...
  Effect.gen(function* () {
    ...
    return handlers
            ...
            data: yield* session
              .create({
                ...
                location:
                  ...
                  (ctx.payload.parentID
                    ? (yield* session.get(ctx.payload.parentID).pipe(
                        Effect.mapError(
                          (error) =>
              ...
              .pipe(
                Effect.catchTag("Session.NotFoundError", (error) =>
                  Effect.fail(
                    new SessionNotFoundError({ sessionID: error.sessionID, message: "Parent session not found" }),
                  ),
                ),
                Effect.catchTag("Session.ChildConflictError", (error) =>
                  Effect.fail(new ConflictError({ message: error.message })),
                ),
              ),
          }
        }),
      ...
      .handle(
        ...
        Effect.fn(function* (ctx) {
          return {
            data: yield* session.recovery(ctx.params.sessionID).pipe(
              Effect.catchTag(
                "Session.NotFoundError",
                (error) =>
                  new SessionNotFoundError({
      ...
      .handle(
        ...
        Effect.fn(function* (ctx) {
          yield* session.resume(ctx.params.sessionID).pipe(
            ...
            Effect.catch((error): Effect.Effect<never, SessionNotFoundError | UnknownError> => {
              if (error instanceof SessionNotFoundError) return Effect.fail(error)
              const ref = `err_${crypto.randomUUID().slice(0, 8)}`
              return Effect.logError("failed to resume session execution", { cause: error }).pipe(
