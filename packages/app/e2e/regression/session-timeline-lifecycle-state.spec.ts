...
test("transitions thinking and hidden reasoning through busy to idle", async ({ page }) => {
  ...
  const thinkingRow = page.locator('[data-timeline-row="Thinking"]')
  await expect(thinkingRow).toBeVisible()
  const thinking = page.getByRole("status")
  await expect(thinking).toBeVisible()
  const orb = page.locator('[data-component="matrix-orb"]')
  await expect(orb).toBeVisible()
  await expect(orb).toHaveAttribute("aria-hidden", "true")
  await expect(orb).toHaveCSS("width", "32px")
  await page.screenshot({ path: "../../plans/validation/matrix-orb-thinking.png" })
