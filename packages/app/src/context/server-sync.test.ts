...
describe("MCP queries", () => {
  test("normalizes canonical capability readiness for the existing MCP controls", async () => {
    const queryClient = new QueryClient()
    const result = await queryClient.fetchQuery(
      loadMcpQuery(ServerScope.local, "/project", {
        list: async () => ({
          data: [
            { name: "browser", state: "available" },
            { name: "xcode", state: "configured" },
            { name: "docs", state: "failed", error: "Connection refused" },
          ],
        }),
    ...
    expect(result).toEqual({
      ...
      xcode: { status: "disabled" },
      docs: { status: "failed", error: "Connection refused" },
    })
  })
