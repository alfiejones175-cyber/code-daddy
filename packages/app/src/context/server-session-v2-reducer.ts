...
import { sessionToolMetadata } from "@/utils/session-message"
      ...
      case "session.tool.progress":
        return updateTool(source, event.data.assistantMessageID, event.data.callID, sessionID, (tool) =>
          tool.state.status === "running"
            ? {
                ...tool,
                state: { ...tool.state, metadata: sessionToolMetadata(event.data) },
              }
            : tool,
        )
        ...
        return updateTool(source, event.data.assistantMessageID, event.data.callID, sessionID, (tool) => {
          ...
          return {
            ...
            state: {
              ...
              metadata: sessionToolMetadata(event.data),
          ...
          return {
            ...
            state: {
              ...
              metadata: {
                ...
                ...sessionToolMetadata(event.data),
              },
              content: event.data.content,
              error: event.data.error,
              // result: event.data.result,
