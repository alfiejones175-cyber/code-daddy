...
export const loadMcpQuery = (
  ...
  >({
    ...
    queryFn: async () => {
      ...
      return api.list({ location: { directory } }).then((result) =>
        Object.fromEntries(
          result.data.map((server) => {
            // The pinned client still describes the earlier MCP status contract.
            if (!("state" in server)) return [server.name, server.status]
            if (server.state === "failed")
              return [
                server.name,
                { status: "failed", error: "error" in server && typeof server.error === "string" ? server.error : "" },
              ]
            return [
              server.name,
              { status: server.state === "available" || server.state === "connected" ? "connected" : "disabled" },
            ]
          }),
        ),
      )
...
export function createServerSyncContextInner(serverSDK: ServerSDK) {
  ...
  const unsub = serverSDK.event.listen((e) => {
    ...
    // Native project appearance changes carry their Location, including agent edits.
    // Update shared metadata even when no directory view is mounted.
    if (event.type === "project.updated") {
      if (directory !== "global")
        applyGlobalEvent({ event, project: globalStore.project, setGlobalProject: setProjects, refresh() {} })
      const project = event.properties
      children.projectIcon(project.worktree, project.icon?.override)
      if (directory !== "global" && directory !== project.worktree)
        children.projectIcon(directory, project.icon?.override)
    }
