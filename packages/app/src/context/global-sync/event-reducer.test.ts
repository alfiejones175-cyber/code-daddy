...
describe("applyGlobalEvent", () => {
  ...
  test("a project snapshot clears removed appearance fields", () => {
    const project = [{ id: "a", name: "Before", icon: { override: "data:image/png;base64,before" } }] as Project[]
    applyGlobalEvent({
      event: { type: "project.updated", properties: { id: "a" } },
      project,
      refresh() {},
      setGlobalProject(next) {
        if (typeof next === "function") next(project)
    ...
    expect(project[0].name).toBeUndefined()
    expect(project[0].icon).toBeUndefined()
  })
...
describe("applyDirectoryEvent", () => {
  test("leaves session-content events to the session reducers", () => {
