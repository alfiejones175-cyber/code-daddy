...
const englishFallbackKeys = new Set([
  ...
  "app.permission.plugin.jev.review.description",
  ...
  "app.settings.updates.row.check.disabledDescription",
  ...
  "ui.ui.genericTool.jev.triage.title",
  "ui.ui.genericTool.jev.ranking.title",
  "ui.ui.genericTool.jev.review.title",
      ...
      for (const locale of domain.locales) {
        ...
        const missing = Object.keys(source).filter(
          (key) => !Object.hasOwn(target, key) && !englishFallbackKeys.has(`${domain.name}.${key}`) &&
            !(domain.name === "app" && key.startsWith("jevReview.")),
        )
