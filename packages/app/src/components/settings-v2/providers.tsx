...
import { providerConnectionKind, useProviderConnections } from "@/hooks/use-provider-connections"
...
export const SettingsProvidersV2: Component<{
  ...
  const popular = createMemo(() => {
    ...
    const items = (
      protocol() === "v1"
        ? providers.popular()
        : connections.list().filter((item) => popularProviders.includes(item.id))
    )
  ...
  const type = (item: ProviderItem) => {
    if (protocol() !== "v1") {
      const kind = providerConnectionKind(connections.connection(item.id))
      if (kind === "key") return language.t("provider.connect.method.apiKey")
      if (kind === "environment") return language.t("settings.providers.tag.environment")
      return connections.methodLabel(item.id) ?? language.t("mcp.status.connected")
    }
  ...
  const canDisconnect = (item: ProviderItem) =>
    protocol() !== "v1" ? connections.connection(item.id)?.type === "credential" : source(item) !== "env"
        ...
        <div class="settings-v2-section" data-component="connected-providers-section">
          ...
          <SettingsListV2>
            <Show
              ...
              fallback={
                <Show when={protocol() === "v1" || (!connections.loading() && !connections.failed())}>
                        ...
                        <span class="settings-v2-provider-env-hint">
                          <Show
                            when={
                              protocol() === "v1"
                                ? source(item) === "env"
                                : connections.connection(item.id)?.type === "env"
                            }
                          >
