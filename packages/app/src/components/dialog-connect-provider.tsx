...
function ProviderPicker(props: {
  ...
  const connections = useProviderConnections(() => props.directory?.())
  const language = useLanguage()
  ...
  return (
    <Show
      when={protocol() === "v1" || (!connections.loading() && !connections.failed())}
      fallback={<ProviderConnectionStatus connections={connections} />}
    >
      <List
        class="px-3"
        ...
        items={() => {
          language.locale()
          return protocol() === "v1"
            ? [{ id: CUSTOM_ID, name: customLabel() }, ...providers.all().values()]
            : connections.list().filter((item) => item.methods.some((method) => method.type !== "env"))
        }}
        filterKeys={["id", "name"]}
  ...
  return (
    <div ref={picker} class="flex min-h-0 flex-1 flex-col gap-4" onKeyDown={handleKeyDown}>
      ...
      <div class="relative min-h-0 flex-1">
        <ProviderConnectionStatus connections={connections} />
        <div class="flex size-full min-h-0 flex-col gap-4 overflow-y-auto pb-8 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          <Show when={rows().length === 0 && !connections.loading() && !connections.failed()}>
...
function ProviderConnection(props: {
  ...
  const protocol = useServerProtocol()
  const alive = { value: true, completed: false, revision: 0 }
  ...
  onCleanup(() => {
    alive.value = false
    if (!alive.completed) cancelAuthorization()
  ...
  function cancelAuthorization() {
    ...
    if (protocol() === "v1" || !authorization) return
    void serverSDK()
  ...
  function formatError(value: unknown, fallback: string): string {
    if (
      ...
      value.kind.startsWith("integration_attempt_")
    )
      return language.t("providers.signInExpired")
    if (value && typeof value === "object" && "data" in value) {
      const nested = formatError(value.data, "")
  ...
  async function selectMethod(index: number, inputs?: Record<string, string>) {
    ...
    if (method.type === "oauth") {
      ...
      const revision = alive.revision
      const server = serverSDK()
      const scope = location()
      const native = protocol() !== "v1"
      await server.api.integration.oauth
        .connect({
          integrationID: props.provider,
          methodID: method.id,
          inputs: inputs ?? {},
        ...
        .then((x) => {
          if (!alive.value || revision !== alive.revision) {
            if (native)
              void server.api.integration.oauth
                .cancel({ integrationID: props.provider, attemptID: x.data.attemptID, location: scope })
                .catch(() => undefined)
            return
          }
        ...
        .catch((e) => {
          if (!alive.value || revision !== alive.revision) return
  ...
  function OAuthCodeView() {
    ...
    async function handleSubmit(e: SubmitEvent) {
      ...
      const result = await serverSDK()
        ...
        .then(() => ({ ok: true as const }))
        .catch((error) => ({ ok: false as const, error }))
      if (!alive.value || store.authorization?.attemptID !== authorization.attemptID) return
      if (result.ok) {
        await complete()
        return
      }
  ...
  function OAuthAutoView() {
    ...
    onMount(() => {
      const poll = async () => {
        ...
        if (["expired", "not_found", "cancelled"].includes(result.status.status)) {
          dispatch({ type: "auth.error", error: language.t("providers.signInExpired") })
          return
        }
