...
describe("normalizeSessionMessages", () => {
  test.each(["pending", "running", "completed"])("rehydrates native %s delegation records", (status) => {
    // HTTP data from the current server, whose tool schema is newer than the bundled client.
    const source = [
        ...
        content: [
          {
            ...
            state: {
              status,
              input: status === "pending" ? '{"description":"Investigate"}' : { description: "Investigate" },
              structured: { sessionID: "ses_child", status: "running" },
              ...(status === "completed" ? { content: [{ type: "text", text: "Findings" }] } : {}),
            },
            time: { created: 2 },
          },
        ],
    ...
    expect(parts).toContainEqual(
      expect.objectContaining({
        tool: "task",
        state: expect.objectContaining({ status, input: { description: "Investigate" } }),
      }),
    )
    if (status !== "pending")
      expect(parts).toContainEqual(
        expect.objectContaining({
          state: expect.objectContaining({ metadata: expect.objectContaining({ sessionId: "ses_child" }) }),
        }),
      )
  })
