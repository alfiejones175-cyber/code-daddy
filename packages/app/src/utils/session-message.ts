...
const decodeToolMetadata = Schema.decodeUnknownOption(Schema.Record(Schema.String, Schema.MutableJson))
...
function normalizeToolMetadata(name: string, metadata: Record<string, unknown>) {
  if (name === "task" && typeof metadata.sessionID === "string") return { ...metadata, sessionId: metadata.sessionID }
...
// Current servers publish structured tool output; retained servers use metadata.
export function sessionToolMetadata(value: { metadata?: unknown; structured?: unknown }) {
  return Option.getOrElse(decodeToolMetadata(value.structured ?? value.metadata), () => ({}))
}
...
function toolPart(sessionID: string, messageID: string, tool: SessionMessageAssistantTool): ToolPart {
  ...
  const state = (() => {
    // Current transcripts use pending; the bundled client still calls it streaming.
    const status: string = tool.state.status
    if (tool.state.status === "streaming" || status === "pending") {
      const raw = typeof tool.state.input === "string" ? tool.state.input : ""
    ...
    if (tool.state.status === "running") {
      return {
        ...
        metadata: normalizeToolMetadata(tool.name, sessionToolMetadata(tool.state)),
        time: { start },
      }
    }
