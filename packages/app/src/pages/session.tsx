...
import { SessionRecoveryNotice } from "@/components/session/session-recovery"
...
import { TranscribeButton } from "@/components/prompt-input/transcribe"
...
export default function Page() {
  ...
  const readOnlyChild = createMemo(() => !!info()?.parentID && serverSDK().protocolKind() !== "v2")
  ...
  const handleKeyDown = (event: KeyboardEvent) => {
    ...
    if (event.key.length === 1 && event.key !== "Unidentified" && !(event.ctrlKey || event.metaKey)) {
      if (composer.blocked() || readOnlyChild()) return
  ...
  const preparePrompt = (text: string) => {
    ...
    if (!current.some((part) => part.type === "text" && part.content.includes(text))) {
      const start = promptLength(current)
      const content = (start ? "\n\n" : "") + text
      prompt.set([...current, { type: "text", content, start, end: start + content.length }], start + content.length)
    }
    requestAnimationFrame(focusInput)
  }

  const openWorkspaceTools = async () => {
    const { WorkspaceTools } = await import("@/components/session/workspace-tools")
    ...
    dialog.show(() => (
      <WorkspaceTools
        ...
        onPreparePrompt={(text) => {
          if (sessionKey() === owner) preparePrompt(text)
        }}
      />
    ))
  }
  const openTeam = async () => {
    const { SessionTeam } = await import("@/components/session/session-team")
    const owner = sessionKey()
    const directory = sdk().directory
    const sessionID = params.id
    if (!sessionID) return
  ...
  command.register("workspace-tools", () => [
    {
      id: "workspace.tools",
      title: language.t("workspaceTools.title"),
      category: language.t("command.category.session"),
      onSelect: openWorkspaceTools,
    },
    {
      id: "workspace.team",
      title: language.t("team.title"),
      category: language.t("command.category.session"),
      onSelect: openTeam,
      disabled: !params.id,
    },
  ...
  const reviewPanelV2Props = () => ({
    ...
    onPrepareFeedback: () => {
      ...
      for (const comment of comments.all()) {
        if (prompt.context.items().some((item) => item.type === "file" && item.commentID === comment.id)) continue
        prompt.context.add({
          type: "file",
          path: comment.file,
      ...
      preparePrompt(language.t("review.feedbackPrompt"))
      showToast({ title: language.t("review.feedbackReady") })
    },
        ...
        actions={
          <Show when={serverSDK().protocolKind() === "v2"}>
            <IconButton
              icon="window-cursor"
              variant="ghost"
              aria-label={language.t("workspaceTools.title")}
              title={language.t("workspaceTools.title")}
