...
export function ReviewPanelV2(props: ReviewPanelV2Props) {
  ...
  return (
    <div class="flex size-full min-h-0 flex-col">
      ...
      <Show when={props.comments?.length && props.onPrepareFeedback}>
        <div class="shrink-0 border-t border-border-weak-base p-2">
          <Button
            class="max-w-full"
            variant="primary"
            onClick={() => props.onPrepareFeedback?.()}
            data-action="prepare-review-feedback"
          >
            {language.t("review.prepareFeedback")}
          </Button>
        </div>
      </Show>
    </div>
