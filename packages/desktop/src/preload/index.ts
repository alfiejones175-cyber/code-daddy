...
import type { DictationEvent } from "@opencode-ai/app/dictation"
...
const dictationCallbacks = new Set<(event: DictationEvent) => void>()
const dictationHandler = (_: unknown, event: DictationEvent) =>
  dictationCallbacks.forEach((callback) => callback(event))
...
const api: ElectronAPI = {
  ...
  dictation: {
    start: (id, locale) => ipcRenderer.invoke("dictation-start", id, locale),
    stop: (id) => ipcRenderer.invoke("dictation-stop", id),
    cancel: (id) => ipcRenderer.invoke("dictation-cancel", id),
    subscribe: (callback) => {
      dictationCallbacks.add(callback)
      if (dictationCallbacks.size === 1) ipcRenderer.on("dictation-event", dictationHandler)
      return () => {
        dictationCallbacks.delete(callback)
        if (dictationCallbacks.size === 0) ipcRenderer.removeListener("dictation-event", dictationHandler)
      }
    },
  },
  updater: {
  ...
  xcodeDetect: () => ipcRenderer.invoke("xcode-detect"),
  xcodeScanProject: (directory) => ipcRenderer.invoke("xcode-scan-project", directory),
  xcodeWorkspaceInspect: (directory) => ipcRenderer.invoke("xcode-workspace-inspect", directory),
  xcodeWorkspaceSimulators: () => ipcRenderer.invoke("xcode-workspace-simulators"),
  xcodeWorkspaceCapture: (id) => ipcRenderer.invoke("xcode-workspace-capture", id),
  legacyMcpPreset: (directory, preset) => ipcRenderer.invoke("legacy-mcp-preset", directory, preset),
  browserWorkspace: {
    navigate: (url, bounds) => ipcRenderer.invoke("browser-workspace-navigate", url, bounds),
    setBounds: (bounds) => ipcRenderer.invoke("browser-workspace-bounds", bounds),
    show: (bounds) => ipcRenderer.invoke("browser-workspace-show", bounds),
    hide: () => ipcRenderer.invoke("browser-workspace-hide"),
    close: () => ipcRenderer.invoke("browser-workspace-close"),
    capture: () => ipcRenderer.invoke("browser-workspace-capture"),
    diagnostics: () => ipcRenderer.invoke("browser-workspace-diagnostics"),
    currentURL: () => ipcRenderer.invoke("browser-workspace-current-url"),
  },
