...
export function registerIpcHandlers(deps: Deps) {
  ...
  const dictation = createDictationBridge({
    helper: helperPath({ packaged: app.isPackaged, app: app.getAppPath(), resources: process.resourcesPath }),
  })
  app.once("will-quit", dictation.clear)
  ...
  ipcMain.handle("dictation-start", (event: IpcMainInvokeEvent, id: string, locale: string) => {
    ...
    const cancel = () => void dictation.cancel(event.sender, id)
    event.sender.once("destroyed", cancel)
    event.sender.once("did-start-navigation", cancel)
    return dictation.start(event.sender, id, locale, () => {
      event.sender.removeListener("destroyed", cancel)
      event.sender.removeListener("did-start-navigation", cancel)
    })
  })
  ipcMain.handle("dictation-stop", (event: IpcMainInvokeEvent, id: string) => {
    assertDictationSender(event)
    if (typeof id !== "string") return
    return dictation.stop(event.sender, id)
  })
  ipcMain.handle("dictation-cancel", (event: IpcMainInvokeEvent, id: string) => {
    assertDictationSender(event)
    if (typeof id !== "string") return
    return dictation.cancel(event.sender, id)
  })
...
function assertDictationSender(event: IpcMainInvokeEvent) {
  const win = BrowserWindow.fromWebContents(event.sender)
  if (
    !win ||
    win.isDestroyed() ||
...
function validDictationInput(id: string, locale: string) {
  if (typeof id !== "string" || id.length === 0 || id.length > 128) throw new Error("Invalid dictation session")
  if (typeof locale !== "string" || locale.length === 0 || locale.length > 64)
    throw new Error("Invalid dictation locale")
  return true
}
