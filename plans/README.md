...
Updated 23 September 2026. Start with the [documentation map](../documentation/README.md) for setup, architecture, and evidence.
...
Model access and release naming are separate. Both the [legacy provider](../packages/opencode/src/provider/provider.ts#L184) and [V2 provider](../packages/core/src/plugin/provider/opencode.ts#L174) have a public/no-key model path driven by provider/catalog data, not the desktop display name. Actual access still depends on the gateway's current model rules.
...
The [Codex workflow plan](codex-alignment.md) records source implementations of `/verify`, `/review`, `/goal`, prompt queueing, MCP management, and background routines. Its [validation record](validation/codex-alignment.md) separates package checks from installed-app behavior. The installed app still uses V1 by default, so V2 goals, durable queueing, MCP management, and routines need a V2 connection and desktop acceptance before they are described as available in the default setup. Sentry remains off pending an explicit privacy decision.

## Next work, in order

| Order | Work | Current state | Completion evidence |
| --- | --- | --- | --- |
| 1 | Keep English correct and restore useful test coverage | Translation backlog is out of scope. Inherited tests still assert non-English parity; earlier review also found omitted package tests and Schema/Client assertion failures. | Follow the [English-only scope](../documentation/localization.md): check English copy/placeholders and scope mandatory language gates accordingly. Include relevant package suites in the Turbo graph, review contract expectations, and check one unchanged snapshot. |
| 2 | Protect free-model access and make model choice clear | Provider support exists; account/live model acceptance remains separate. | Preserve the `opencode` provider and catalog refresh; clearly label free and paid models and identify the model selected for a request. Verify currently available free models remain selectable in public and connected states, and an unavailable free model never silently switches to a paid one. Exercise an actual selected model separately from deterministic CI. |
| 3 | Review current Jev findings | User testing is in progress. Existing CLI, triage/ranking tools, and local tests are implemented. | Triage the [Core](validation/jev-audit-core.md), [platform](validation/jev-audit-platform.md), and [response-diagnostic](validation/jev-audit-response-diagnostic.md) evidence. Separate confirmed defects, migration gaps, and uncertain advice; reproduce before implementing fixes. |
| 4 | Integrate Jev output review into the app | Output-review UI and API work is present in the working tree; it still needs end-to-end acceptance before being called shipped. This remains ahead of skill/action selection. | Complete the staged [app integration plan](jev-integration.md): review a selected completed response against its requirements and supplied evidence; preserve typed findings and provenance; show unavailable and stale states; and prepare an editable follow-up for the main agent. Keep automatic review opt-in until it proves useful on real tasks. |
| 5 | Unify task results and recovery | Diff/review, delegated chats, and explicit recovery exist. A consolidated evidence view is proposed. | Give each task one wrap-up showing changed files, actual check results, artifacts, delegated-task outcomes, and Jev findings. Clearly separate verified outcomes from advisory findings. For interrupted work, summarize what completed, possible side effects, and next steps; resume only after explicit review, never by silently replaying provider work. Verify interruption/reconnect/reopen continuity and invalidate outdated review evidence. |
| 6 | Make the private build dependable on the actual machines used | Local development and a few friends' installations are the target. | Verify startup, provider connection, sessions, dictation if used, and installation/update behavior on those machines. Make the installed version and update path clear, preserve settings and history during updates, and document how friends install and update the custom build. No public release pipeline is required. |
...
2. **Implemented in source:** a shared output-review operation with versioned narrow rubrics and deterministic fixtures.
3. **Implemented in source; acceptance pending:** an explicit app review action, evidence/results UI, and an editable “address findings” draft.
...
Detailed contracts, evaluation criteria, and transport choices live in the [Jev integration plan](jev-integration.md).
