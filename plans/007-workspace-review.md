...
**Scope update, 22 September 2026:** the user clarified this is an English-only app for personal use and a few friends. The [current roadmap](/Users/alfredo/Documents/code-daddy/plans/README.md) supersedes the multilingual repair, independent public-release identity, market-validation, and broad distribution recommendations below. Keep OpenCode provider access, Jev output review, English correctness, and session/data reliability. The [language policy](/Users/alfredo/Documents/code-daddy/documentation/localization.md) makes non-English completeness optional. Findings and test counts below remain the historical review snapshot; a translation-parity failure is no longer an English-product defect.
...
| --- | --- | --- | --- |
| P1 | The normal Turbo test run omits substantial existing test coverage. Failures can survive the main CI unit-test gate. | [Turbo configuration](/Users/alfredo/Documents/code-daddy/turbo.json:5), [CI invocation](/Users/alfredo/Documents/code-daddy/.github/workflows/test.yml:66), and an actual `turbo run test --dry=json`. Only six runnable test tasks were planned. | Every intended test-bearing package appears in the dry-run graph and executes in CI. Explicitly document exclusions. Check the graph automatically when adding a package. |
| P1 | Schema and Client tests already fail when run directly. Contract changes and their expectations are out of sync. | [Schema manifest test](/Users/alfredo/Documents/code-daddy/packages/schema/test/event-manifest.test.ts:12): expected 55 server events, observed 58; a positional identity assertion also fails. [Client public surface test](/Users/alfredo/Documents/code-daddy/packages/client/test/promise.test.ts:7): additional `server.project` and `server.mcp` groups. | Review the intended public contract, then update implementation or expectations accordingly. All contract/identity/import-boundary tests pass through CI. Do not blindly update counts to make the tests green. |
| P1 before distribution | Code Daddy is currently the development-channel identity; beta/prod retain upstream OpenCode identity and release destinations. | [Packaging](/Users/alfredo/Documents/code-daddy/packages/desktop/electron-builder.config.ts:39), [release channels](/Users/alfredo/Documents/code-daddy/packages/desktop/electron-builder.config.ts:146), [app identity and data directory](/Users/alfredo/Documents/code-daddy/packages/desktop/src/main/index.ts:125). | Distinct app IDs, data directories, protocol handling, signing configuration, release destinations, and documented migration. Test installation alongside OpenCode, update, rollback, and uninstall without affecting the other application. |
| P2 | The app's localization gate is red. Newly added workflows are not consistently translated. | [Parity test](/Users/alfredo/Documents/code-daddy/packages/app/src/i18n/parity.test.ts:112). Latest run: 767 pass, 1 fail, first reported locale `ar`, including workspace/capability/team strings. | Restore dictionary and placeholder parity across supported locales using the repository's required translation sources. Add long-label and RTL acceptance for newly added controls. |
| P2 | CI's intended Node 24.15 pin for browser installation is overwritten by the subsequent shared setup action. | [E2E pin](/Users/alfredo/Documents/code-daddy/.github/workflows/test.yml:104) followed by [shared Node 24 setup](/Users/alfredo/Documents/code-daddy/.github/actions/setup-bun/action.yml:13). | Parameterize the shared action or preserve the selected Node version; print and assert the actual version immediately before Playwright installation. This is a configuration defect; no hosted CI hang was reproduced here. |
| P2 | The app and Session UI use a vendored 1.17.13 client while the workspace packages are 1.18.31. New endpoints need handwritten compatibility code. | [App dependency](/Users/alfredo/Documents/code-daddy/packages/app/package.json), [Session UI dependency](/Users/alfredo/Documents/code-daddy/packages/session-ui/package.json), [appearance transport](/Users/alfredo/Documents/code-daddy/packages/app/src/utils/server.ts:70), [protocol adapter](/Users/alfredo/Documents/code-daddy/packages/app/src/utils/server-compat.ts). | Define a supported client/server matrix and a migration owner. Integrate the current generated client in a bounded change, retain tested V1 adapters where required, and remove redundant endpoint shims only after equivalent tests pass. |
| P2 | Canonical-looking runtime documentation contradicts implemented behavior. Future developers and agents can follow the wrong plan. | [Session spec](/Users/alfredo/Documents/code-daddy/specs/v2/session.md:153) says watchdog policy is deferred; [runner](/Users/alfredo/Documents/code-daddy/packages/core/src/session/runner/llm.ts:269) configures first-event/inactivity deadlines. The spec also says applying the agent system prompt remains work, while the runner includes it at line 243. | Reconcile the capability/parity table with code and current tests. Give each remaining gap an owner and evidence. Update the same document when implementation changes. |
| P2, small UX repair | The primary Open project label truncates in both inspected desktop and compact captures. A first-run action loses clarity. | Saved [1280×800 desktop](/Users/alfredo/Documents/code-daddy/plans/validation/spectrum-sidebar-desktop-dark.png) and [390×844 compact](/Users/alfredo/Documents/code-daddy/plans/validation/spectrum-sidebar-compact.png); [equal-width action grid and ellipsis](/Users/alfredo/Documents/code-daddy/packages/app/src/pages/layout/project-sidebar.css:131). | Show the complete primary action label at those widths and in longer translations, without reducing text size. Verify the accessible name and keyboard path. These captures are existing synthetic fixtures, not fresh native screenshots. |
...
| --- | --- | --- |
| [Project Effect skill](/Users/alfredo/Documents/code-daddy/.opencode/skills/effect/SKILL.md) | Core, Protocol, Server, context, or runner changes. | APIs checked against source and local examples; explicit layer ownership; cancellation and durable-state acceptance. Its reference source should match the pinned Effect dependency/patches, rather than an unrelated moving revision. |
...
| [RTL-aware development](/Users/alfredo/Documents/code-daddy/.opencode/skills/rtl-aware-development/SKILL.md) | Fixing localization and testing compact navigation or long translated controls. | Logical layout direction, correct labels, and RTL interaction acceptance alongside corpus-verified translations. |
...
Skills available to the assistant in this Codex session are not automatically installed inside Code Daddy. The V2 [skill configuration](/Users/alfredo/Documents/code-daddy/packages/core/src/config.ts:94) accepts additional paths/URLs, and the [skill plugin](/Users/alfredo/Documents/code-daddy/packages/core/src/config/plugin/skill.ts:23) discovers skill directories. Put portable, repository-owned guidance in `.opencode/skills`, or explicitly configure supported paths. Do not copy Codex-specific tool instructions into Code Daddy and assume their tools will exist there.
